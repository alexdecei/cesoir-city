...
- `NOMINATIM_ENDPOINT` – override Nominatim endpoint (default `https://nominatim.openstreetmap.org/search`)
...
For `--city "Paris"` (case-insensitive, FR default), the tool intentionally uses the **Métropole du Grand Paris** perimeter (OSM relation `5814660`) instead of intramuros, to match the Préfecture scope.
...
### Resolve missing venues via Nominatim

Given a JSON list of missing venues (nom/adresse/city), resolve them against OSM using Nominatim:

```bash
npm run osm:resolve-missing -- --in ./out/missing_venues.json --country FR --out ./out/resolve
```

Outputs:

- `resolved.found_by_name.json`
- `resolved.found_by_address.json`
- `resolved.not_found.json`
- `venues.db.jsonl` – DB-shaped venues ready to upsert
- `inject.sql` – SQL UPSERT/INSERT script
- `report.json`
