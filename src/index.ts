...
import { runResolveMissing } from './osm/resolve_missing.js';
...
async function runOsmFetchCommand(options: {
  ...
  const report = {
    ...
    selectedAreaId: summary.stats.selectedAreaId,
    selectedRelationId: summary.stats.selectedRelationId,
    selectedAreaName: summary.stats.selectedAreaName,
    parisSpecialCase: summary.stats.parisSpecialCase,
...
program
  .command('osm:resolve-missing')
  .requiredOption('--in <path>', 'Path to missing venues JSON')
  .option('--out <dir>', 'Output directory', DEFAULT_OUTPUT_DIR)
  .option('--country <code>', 'Country code (default FR)', 'FR')
  .option('--concurrency <value>', 'Nominatim concurrency (default 1)')
  .option('--delay-ms <value>', 'Delay between requests in ms (default 1000)')
  .option('--name-threshold <value>', 'Name match score threshold (default 80)')
  .option('--address-threshold <value>', 'Address match score threshold (default 80)')
  .option('--dry-run', 'No-op flag (resolve-missing never mutates DB)', true)
  .action(async (opts: {
    in: string;
    out?: string;
    country?: string;
    ...
    addressThreshold?: string;
  }) => {
    try {
      await runResolveMissing({
        inputPath: opts.in,
        outDir: path.resolve(opts.out ?? DEFAULT_OUTPUT_DIR),
        country: opts.country ?? 'FR',
        concurrency: getNumber(opts.concurrency, 1),
        delayMs: getNumber(opts.delayMs, 1000),
        nameThreshold: getNumber(opts.nameThreshold, 80),
        addressThreshold: getNumber(opts.addressThreshold, 80),
      });
    } catch (error) {
      logger.error({ err: error }, 'OSM resolve-missing command failed');
      process.exitCode = 1;
    }
  });
