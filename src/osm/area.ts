...
export function isParisSpecialCase(city: string, country?: string): boolean {
  const normalizedCity = normalizeCityName(city);
  const normalizedCountry = country?.toLowerCase();
  return normalizedCity === 'paris' && (!normalizedCountry || normalizedCountry === 'fr');
}

export function resolveParisSpecialCase(): AreaSearchResult {
  const relationId = 5_814_660;
  ...
  return {
    ...
    areaId,
    relationId,
    areaName: 'Métropole du Grand Paris',
    parisSpecialCase: true,
  };
}

export function parisSpecialCaseSelfCheck(): boolean {
  const result = resolveParisSpecialCase();
  return isParisSpecialCase('Paris', 'FR') && result.areaId === 3_605_814_660;
}

export async function findAdminArea(options: AreaSearchOptions): Promise<AreaSearchResult> {
  if (isParisSpecialCase(options.city, options.country)) {
    return resolveParisSpecialCase();
  }
  ...
  if (candidates.length === 0) {
    return { selected: null, candidates: [], ambiguous: false, parisSpecialCase: false };
  }

  const normalizedTarget = normalizeCityName(options.city);
  ...
  const relationId = selected?.id;
  const areaId = relationId ? relationId + 3_600_000_000 : undefined;
